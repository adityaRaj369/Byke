  ...
  const verifyOTP = async () => {
    ...
    try {
      ...
      const response = await api.post('/auth/rider/verify-firebase-token', {
        idToken, mobileNumber: `+91${phone}`, fullName: `Rider ${phone.slice(-4)}`,
      });
      const { accessToken, refreshToken, userId, fullName, profilePhotoUrl } = response.data;
      const userPayload = { id: String(userId), name: fullName || `Rider ${phone.slice(-4)}`, phone: `+91${phone}`, profilePhoto: profilePhotoUrl || undefined, role: 'rider' as const };
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.content}>
          ...
          <View style={styles.header}>
            <Text style={styles.title}>{otpSent ? 'Verify OTP' : 'Rider Login'}</Text>
