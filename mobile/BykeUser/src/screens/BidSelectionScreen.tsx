...
const BidSelectionScreen = () => {
  ...
  const fetchBids = async () => {
    try {
      ...
      setBids(response.data.filter((bid: Bid) => bid.status === 'PENDING'));
    } catch (error) {
    ...
    <View style={styles.bidCard}>
      <View style={styles.riderHeader}>
        ...
        <View style={styles.riderInfo}>
          <Text style={styles.riderName}>{item.rider.user.fullName}</Text>
          <View style={styles.ratingRow}>
            <Star size={14} color="#EAB308" fill="#EAB308" />
            <Text style={styles.ratingText}>
              {item.rider.averageRating.toFixed(1)} ({item.rider.totalRides} rides)
            </Text>
          </View>
        </View>
      ...
      <View style={styles.vehicleInfo}>
        <Text style={styles.vehicleText}>
          {item.rider.vehicleType} • {item.rider.vehicleModel}
        </Text>
        <Text style={styles.vehicleNumber}>{item.rider.vehicleRegistrationNumber}</Text>
      </View>
